...
class ExportW3DBFME(bpy.types.Operator, ExportHelper, ReportHelper):
    """Export to Westwood 3D file format (.w3d/.w3x)"""
    bl_idname = 'export_mesh.westwood_w3d'
    bl_label = 'Export W3D'
    bl_options = {'UNDO', 'PRESET'}
...
def menu_func_exportb(self, _context):
    self.layout.operator(ExportW3DBFME.bl_idname, text='BFME1 Animation (.w3d/.w3x)')
...
def register():
    ...
    bpy.types.TOPBAR_MT_file_export.append(menu_func_exportb)
...
def unregister():
    ...
    bpy.types.TOPBAR_MT_file_export.remove(menu_func_exportb)
